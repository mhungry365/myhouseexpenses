...
function OnboardScreen({user,onDone}){
  const [screen,setScreen]=useState("choose"); // choose|create|join
...
// ── HOUSE APP ─────────────────────────────────────────────────────
function HouseApp({myPerson,myHouse,isAdmin,onSignOut}){
...
function HouseAdminView({house,persons,bills,categories,reload,showToast}){
  ...
  return(
    <div style={{padding:"0 16px"}}>
      ...
      <p style={{fontSize:13,color:"#64748b",margin:"0 0 16px"}}>Share the code above with housemates to invite them.</p>
    ...
    <div>
      <div style={{margin:"0 16px 20px",borderRadius:20,background:"#0f172a",padding:"24px 20px",color:"white"}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <button style={{flex:1,padding:"13px",borderRadius:12,background:"white",border:"none",color:"#0f172a",fontWeight:700,fontSize:15,cursor:"pointer"}}>Settle Up</button>
